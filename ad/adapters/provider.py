...
class CreateProviderOlx(CreateProvider):
    ...
    def get_raw(self) -> List[Tuple]:
        ...
        #     html = f.read()
        dom = etree.HTML(html)
        return [self.process_item(item) for item in
                dom.xpath('.//div[contains(@data-cy, "l-card")]')]
    ...
    def process_item(item):
        # TODO parse per item
        title, dirty_price, *_ = item.xpath('.//p[contains(@class, "Text")]/text()') # ['сдам квартиру в днепре 12 квартал', '3 000 грн.', 'Днепр', '11 октября 2021 г.', '36 м²']
